...
/**
 * Configuration component shared by {@link SlingMainServlet} and {@link RequestParameterSupportConfigurer} for
 * backward compatibility of request parameter options
 */
@Component(name = RequestParameterConfig.PID, service = RequestParameterConfig.class)
@Designate(ocd = RequestParameterConfig.Config.class)
public class RequestParameterConfig {
    ...
    private void activate(Config config) {
        ...
        if (log.isInfoEnabled()) {
            ...
            final String fileLocation = resolveLocation();
    ...
    /**
     * Resolve the file location for temp files
     * @return the resolved file location
     */
    public @Nullable String resolveLocation() {
        String fileLocation = config.file_location();
        if (fileLocation != null) {
            File file = new File(fileLocation);
            if (!file.isAbsolute()) {
                file = new File(this.settingsService.getSlingHomePath(), fileLocation);
